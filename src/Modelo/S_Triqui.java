...
public class S_Triqui implements MouseListener {
    ...
    public void comprobar() {
        Analizar an = new Analizar(this.jugada);
        int n = an.analizar();
        if (n != 0) {
            se.sf.enviarDatos("" + n);
            if (n == 1) {
    ...
    public void colocarJuegoVentana() {
        ...
        this.se.contenido.add(juego, BorderLayout.CENTER);
    }
    ...
    public void mouseEntered(MouseEvent me) {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (me.getSource() == fichas[i][j] && jugada[i][j] == 0 && fichas[i][j].getIcon() == null) {
